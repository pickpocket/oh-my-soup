 ...
 * immediate-mode hosts all behave as in production — with `OMS_TUI_DEBUG`
 * pointed at the unix socket served by an oms/pi-tui host. Injected input
...
/** Minimal slice of the host schema builder (`oms.zod`) this tool uses. */
interface Schema {
...
function loadVt() {
	...
	vtApi ??= import("kitty-vt-wasm").catch((error) => {
		vtApi = null;
		throw new Error(
			`screen emulation needs kitty-vt-wasm — run \`bun install\` in .oms/tools (${error})`,
		);
	});
	return vtApi;
}
...
async function loadCanvas() {
	...
	try {
		...
		throw new Error(
			`shot needs @napi-rs/canvas — run \`bun install\` in .oms/tools (${error})`,
		);
	}
 ...
 * session — oms-tui host or not — can be read as plain text (`snapshot`) or
		...
		return Promise.reject(
			new Error(
				`session "${session.name}" has no debug socket — the app is not an ` +
					"oms-tui host (or exited). screen/raw/send/resize/stop still work.",
			),
		);
	}
...
async function stopSession(session: Session): Promise<number | null> {
	try {
		if (session.sock) {
			...
			// Non-oms-tui apps have no debug socket; Ctrl-C is the
...
const factory = (oms: ToolHost) => {
	const startSession = async (params: TuiParams): Promise<string> => {
		...
		if (connected) {
			...
			text +=
				"\n(no debug socket: app is not an oms-tui host; `send` injects " +
	...
	return {
		...
		label: "TUI Debug",
		description:
			"Run and debug oms/pi-tui apps headlessly on a real PTY plus the " +
			"OMS_TUI_DEBUG socket. Start defaults to oms itself " +
			"(packages/coding-agent/src/cli.ts); override with file (a TS/JS entry, e.g. " +
			"file: \"packages/tui/examples/debug-demo.ts\") or bin (an executable name/path), plus optional rows/cols and args. Any oms/pi-tui app serves " +
			"OMS_TUI_DEBUG. Ops: text (viewport screenshot as plain text), screen " +
		...
		parameters: oms.zod.object({
			...
			file: oms.zod
				.string()
				.optional()
				.describe("start: TS/JS entry path relative to cwd (default: packages/coding-agent/src/cli.ts — oms itself)"),
			bin: oms.zod
				.string()
				.optional()
				.describe("start: executable name or path"),
			args: oms.zod
				.array(oms.zod.string())
				.optional()
			...
			x: oms.zod.number().optional().describe("mouse: zero-based column"),
			y: oms.zod.number().optional().describe("mouse: zero-based viewport row"),
			action: oms.zod
				.string()
				.optional()
		...
		async execute(
			...
			switch (params.op) {
				...
				case "shot": {
					...
					const png = join(
						tmpdir(),
						`oms-tui-${session.name}-${Date.now()}.png`,
					);
