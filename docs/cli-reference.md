...
When the first non-flag argument is **not** a registered subcommand, `oms`
routes to the default [`launch`](#launch-the-default-command) command and treats
the arguments as the initial prompt. So `oms "fix the build"` launches a session
with that message, while `oms models` runs the `models` subcommand.
...
`--print` / `-p` runs `oms` non-interactively: it processes the prompt, streams
...
| `acp` | Run Oh My Soup as an ACP (Agent Client Protocol) server over stdio. | [approval mode](./approval-mode.md#acp-sessions) |
...
| `worktree`, `wt` | List or clear agent-managed git worktrees (`~/.oms/wt`). | |
