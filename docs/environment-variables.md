...
| `COPILOT_INTEGRATION_ID`  | GitHub Copilot client identity override         | Default `copilot-chat` (chat surface). Denied requests retry once as the Copilot CLI; set to pin the client id and skip the retry ([#11372](https://github.com/pickpocket/oh-my-soup/issues/11372)) |
...
| `PI_WALK_WORKERS`            | Filesystem walker worker count used by the `oms grep` walker and related parallel traversal (`pi-walker`; default `4`; `0` = auto-detect, `1` = serial). Renamed from `PI_GREP_WORKERS`; the old name is no longer read                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `OMS_WORKTREE_DIR`                                  | Agent-managed worktrees directory override (default `~/.oms/wt`); must be absolute or `~`-relative, relative paths are ignored; wins over the `worktree.base` setting                      |
| `OMS_GITHUB_CACHE_DB`                               | Overrides the GitHub view cache database path (default `~/.oms/cache/github-cache.db`)                                                                                                     |
