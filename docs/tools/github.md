...
- Subprocesses / native bindings
  ...
  - `pr_checkout` and `pr_push` also invoke git operations via `@oh-my-soup/pi-natives/vcs` (`vcs.requireGit()`), serialized by `withRepoLock()` from `packages/coding-agent/src/utils/repo-lock.ts`.
