...
- Filesystem
  ...
  - Session files are named `<ISO-timestamp-with-:-and-.-replaced>_<uuidv7>.jsonl` in the session directory; default directory selection is `~/.oms/agent/sessions/<encoded-cwd>/` when no override is passed.
