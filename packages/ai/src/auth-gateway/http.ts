 ...
 *
 * pi-native oms clients send `x-oms-install-id` / `x-oms-hostname` /
 * `x-oms-app` (see `providers/pi-native-client.ts`); any client may set them.
...
export function resolveClientIdentity(headers: Headers): ClientUsageIdentity {
	...
	const installId = read("x-oms-install-id");
	const app = read("x-oms-app");
	if (!installId) {
		return { installId: getInstallId(), hostname: os.hostname(), app: app ?? "gateway" };
	}
	return { installId, hostname: read("x-oms-hostname"), app: app ?? "gateway" };
}
