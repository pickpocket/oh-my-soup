...
describe("resolveOpenAIRequestSetup User-Agent", () => {
	test("sets oms User-Agent on xAI when none is provided", () => {
...
describe("xAI stream User-Agent", () => {
	test("xAI Responses POST sends oms User-Agent", async () => {
		...
		expect(captured.userAgent).toMatch(/^oms\/\d+\.\d+\.\d+$/);
	});

	test("xAI OAuth Responses POST sends oms User-Agent", async () => {
		const captured = await captureStreamHeaders(
	...
	test("OpenAI Completions POST does not send oms User-Agent", async () => {
		const captured = await captureStreamHeaders(
