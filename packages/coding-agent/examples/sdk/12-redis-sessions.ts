 ...
 * `BlobStore` keep writing to `~/.oms/agent/...`. Reach for an object store
 * (S3, R2, GCS) if you need those off-host too.
 */

import { createAgentSession, RedisSessionStorage, SessionManager } from "@oh-my-soup/pi-coding-agent";
...
const storage = await RedisSessionStorage.create({
	client: redis,
	prefix: "oms:sessions:", // optional, this is the default
});
