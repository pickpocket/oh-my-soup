/**
 * Bun `--preload` shim for the oms dev launcher (`scripts/oms`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the oms CLI: Bun reads
 ...
 * import-time snapshots (e.g. `getProjectDir()` in `@oh-my-soup/pi-utils/dirs`).
 */
const launchCwd = process.env.OMS_LAUNCH_CWD;
if (launchCwd) {
	delete process.env.OMS_LAUNCH_CWD;
	try {
