 ...
 * project providers like `.claude/settings.json`, never re-derived from `.oms`
...
export interface SourceMeta {
	 ...
	 * `"oms"` for oms's own registry, `"plugin-dir"` for `--plugin-dir`). Lets
	 * user-scope gating distinguish oms's own installs from the foreign Claude
