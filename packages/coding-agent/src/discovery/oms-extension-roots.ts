...
export type OmsExtensionRootMode = ExtensionRootMode;
 ...
 * outside a {@link withOmsExtensionRootScope} callback.
 */
 ...
 *    from a partial `.oms` disk scan; scopeless callers read the persisted
 *    `.oms` config, which supplies its own level.
...
export async function listOmsExtensionRoots(ctx: LoadContext): Promise<OmsExtensionRoot[]> {
	...
	const rootMode: OmsExtensionRootMode = ctx.extensionRoots?.mode ?? scopedRoots?.mode ?? injectedCliRootMode;
	...
	if (rootMode === "merge") {
		...
		// read the persisted `.oms` config on disk, which is the authoritative
