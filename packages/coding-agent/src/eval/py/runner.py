...
_TOOL_BRIDGE_MARKER = "__oms_tool_bridge__"
...
def _emit_shadow_plan(req: dict) -> None:
    ...
    # cell binds `__oms_with_call_site__` anywhere, authoritative execution
...
_CALL_SITE_HELPER_NAME = "__oms_with_call_site__"
...
def _restore_call_site_helper(ns: dict) -> None:
    ...
    A previous cell can assign or delete `__oms_with_call_site__`; without
...
def _handle_tool_request(req: dict) -> None:
    ...
    try:
        tools = _STATE.user_ns.get("__oms_tools__") or {}
        op = req.get("op")
        if op == "describe":
...
async def _handle_request_async(req: dict) -> None:
    ...
    reset_call_occurrences = _STATE.user_ns.get("__oms_reset_call_occurrences__")
