...
/**
 * Apply a pending compiled-binary update in a detached child so the running
 * TUI never sees updater output. The child invokes the current OMS binary's
 * update command, which owns platform-specific replacement semantics.
 */
...
async function runInteractiveMode(
	...
	try {
		...
		checkedVersionPromise.then(async newVersion => {
			if (!settings.get("startup.checkUpdate") || !newVersion) {
				return;
			}
			if (settings.get("update.auto") && $env.PI_COMPILED === "true") {
				const applied = await runBackgroundSelfUpdate();
				if (applied) {
					mode.showStatus("Update " + newVersion + " installed. Restart OMS to use it.");
					return;
				}
			}
