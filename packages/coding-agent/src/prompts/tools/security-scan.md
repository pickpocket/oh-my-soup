OMS-native repository security scans: plan, start, inspect, cancel, validate.
`preflight`: immutable plan pinned to repository snapshot, model, exact OAuth credential.
`start`: plan → background OMS job.
...
`cloud_pull`: cloud findings → canonical OMS security store, available through `security://`.
