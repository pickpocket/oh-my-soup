...
export interface CreateAgentSessionOptions {
	 ...
	 * unchanged so their family serializes onto one browser page.
	 */
...
function createCustomToolContext(ctx: ExtensionContext): CustomToolContext {
	return {
		...
		searchBrowserSessionId: ctx.sessionManager.getSessionId(),
	};
}
	...
	try {
		...
		const rebuildSystemPrompt = async (
			...
			const defaultPrompt = await buildSystemPromptInternal({
				...
				// Forced-think guidance flavor: mirrors the external-thinking
				// tool-choice gate (settings toggle + live-model support). The
				// template renders the optional-scratchpad copy otherwise.
				thinkForced:
					settings.get("externalThinking") === true && supportsExternalThinking(agent?.state.model ?? model),
				activeRepoContext,
			});
