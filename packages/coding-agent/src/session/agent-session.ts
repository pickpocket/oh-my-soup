...
export class AgentSession {
	...
	constructor(config: AgentSessionConfig) {
		...
		const scheduledPromptsHost: ScheduledPromptSchedulerHost = {
			...
			deliver: async (_job, promptText, mode) => {
				// Idle starts a turn directly; a live turn queues through the
				// session's normal steer/follow-up path.
	...
	async #reconcileModelDependentState(previousModel: Model | undefined, model: Model): Promise<void> {
		...
		// The think-guidance flavor (optional vs forced) follows the model's
		// external-thinking support. A switch across that boundary must
		// re-render the base prompt, or the previous model's flavor goes stale
		// (e.g. a dial-less reasoner's optional copy surviving onto a model
		// whose replacement forces the scratchpad). A model switch already
		// busts the provider prompt cache, so the rebuild is cache-neutral.
		if (
			this.getActiveToolNames().includes("think") &&
			supportsExternalThinking(previousModel) !== supportsExternalThinking(model)
		) {
			try {
				await this.refreshBaseSystemPrompt();
			} catch (error) {
