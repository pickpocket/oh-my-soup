...
export class SessionMaintenance {
	...
	/**
	 * Saved notes are never sacrificed to context pressure. Reclaim room from
	 * tool results first, then the dead-end rescue ladder, then an available
	 * summary compaction. Only a request that still cannot fit reports the
	 * durable-note-safe failure.
	 */
	async #recoverImportantNotesFit(pendingMessages: AgentMessage[], allowCompaction: boolean): Promise<void> {
		...
		if (
			...
			hasUsableCompactionMethod("threshold", model, compactionSettings) &&
	...
	#projectPreSnapcompactContextTokens(preparation: CompactionPreparation): number {
		const opts = { excludeEncryptedReasoning: true } as const;
		let tokens = this.#fixedContextTokens();
	...
	async runPrePromptCompactionIfNeeded(messages: AgentMessage[]): Promise<void> {
		...
		if (
			...
			// persisted oversized turn; an elide can create the missing cut point.
			await this.#recoverImportantNotesFit(messages, true);
			return;
		}
		...
		// The note reference survives every compaction. Fail before a futile
	...
	async maintainContextMidRun(
		...
		// Futile-floor gate mirrors the pre-prompt path: notes plus prompt
	...
	#projectCompactionContextTokens(args: CompactionProjectionArgs): number {
		const nonMessageTokens = this.#fixedContextTokens();
		const branch = this.#host.sessionManager.getBranch();
