...
/**
 * Cap retained display-only tool snapshots per tool. Yield payloads are
 * correctness-bearing (incremental sections assemble the final result), so
 * they remain unbounded for the duration of a subagent run.
 */
...
export async function finalizeSubagentLifecycle(args: {
	...
	// The idle window keeps the whole live session; the SSE debug ring contains
	// only last-request diagnostics with no reader on an idle subagent.
...
export async function runSubprocess(options: ExecutorOptions): Promise<SingleResult> {
	...
	const runSubagent = async (): Promise<{
		...
		try {
			...
			monitor.setActiveSession(session);
			// Native sessions publish authoritative run-state notifications before
			// the deferred wire-level `agent_end`. Lightweight executor embedders
			// intentionally provide only the prompt/dispose surface, so retain
			// their compatibility instead of treating this registry optimization
			// as a required session capability.
			const registry = AgentRegistry.global();
			if (typeof session.subscribeRunState === "function") {
