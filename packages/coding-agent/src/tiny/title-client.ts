...
import { $env, getTinyWorkerRuntimeDir, logger, prompt } from "@oh-my-soup/pi-utils";
...
/** Detach a worker so it outlives this oms process; its output goes to a per-worker log file. */
...
function mlxLaunch(modelKey: TinyLocalModelKey, emitProgress: (event: TinyTitleProgressEvent) => void): WorkerLaunch {
	...
	return {
		...
		async spawn(endpoint, logPath) {
			...
			const script = await stageRunnerScript("oms-tiny-mlx", "py", MLX_SERVER_SCRIPT);
			const env = inferenceWorkerEnv({
				PYTHONUNBUFFERED: "1",
...
async function logTail(logPath: string): Promise<string> {
	try {
		...
		return text
			.split("\n")
			.filter(line => !line.startsWith("oms tiny worker listening on "))
			.join("\n")
			.trim()
			.slice(-500);
	} catch {
 ...
 * replacing it when its launch tag is stale. A concurrent oms process may win
...
export async function connectTinyWorker(
	...
	while (Date.now() < deadline) {
		...
		if (spawned.proc.exitCode !== null) {
			// Our child is gone: either it lost the bind race to a sibling oms
