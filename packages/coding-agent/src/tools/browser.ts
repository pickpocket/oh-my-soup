...
import type { AgentToolContext, AgentToolResult, AgentToolUpdateCallback } from "@oh-my-soup/pi-agent-core";
...
import * as tabSupervisor from "./browser/tab-supervisor";
...
/**
 * Direct browser-tool facade for host callers that invoke the browser outside
 * an eval prelude. It shares the prelude implementation so both surfaces use
 * one browser lifecycle and configuration path.
 */
export class BrowserTool {
	...
	execute(
		...
		return invokeBrowser(this.session, params, {
...
function sweepIdleOwnedTabs(session: ToolSession): Promise<number> {
	...
	if (!(idleSec > 0)) {
		tabSupervisor.cancelIdleCloseForOwner?.(ownerId);
		return Promise.resolve(0);
	}
	const releaseIdleTabsForOwner = tabSupervisor.releaseIdleTabsForOwner;
	if (!releaseIdleTabsForOwner) return Promise.resolve(0);
...
function sameBrowserKind(a: BrowserKind, b: BrowserKind): boolean {
	...
	switch (a.kind) {
		...
		case "spawned": {
			if (b.kind !== "spawned" || a.path !== b.path) return false;
			const aArgs = a.args ?? [];
			const bArgs = b.args ?? [];
			return aArgs.length === bArgs.length && aArgs.every((arg, index) => arg === bArgs[index]);
		}
		case "connected":
