...
import { $which, getCamoufoxDir, getPuppeteerDir, logger, removeWithRetries } from "@oh-my-soup/pi-utils";
...
// Keep Camoufox's native dependency graph out of ordinary Chromium startup;
// pin its engine cache before the lazy package resolution used by web search.
...
const CAMOUFOX_TERRITORY_INFO: CamoufoxAsset = {
	moduleSpecifier: "camoufox-js/dist/data-files/territoryInfo.xml",
...
const CAMOUFOX_WEBGL_DATA: CamoufoxAsset = {
	moduleSpecifier: "camoufox-js/dist/data-files/webgl_data.db",
...
function resolveCamoufoxAssetSource(asset: CamoufoxAsset): string {
	return fileURLToPath(import.meta.resolve(asset.moduleSpecifier));
}
...
async function materializeCamoufoxAsset(asset: CamoufoxAsset): Promise<string> {
	...
	try {
		bytes = new Uint8Array(await Bun.file(resolveCamoufoxAssetSource(asset)).arrayBuffer());
	} catch (error) {
...
export async function smokeTestCamoufoxAssets(): Promise<void> {
	...
	const [territoryInfo, webglData] = await Promise.all([
		fs.promises.readFile(territoryInfoPath),
		fs.promises.readFile(webglDataPath),
	]);
...
export async function ensureCamoufoxEngine(): Promise<string> {
	...
	camoufoxEnginePromise = (async () => {
		// Camoufox's optional native dependency tree loads only when a browser-backed search needs it.
		...
		try {
			...
			try {
				...
				throw new ToolError(`Failed to install the Camoufox engine: ${(error as Error).message}`);
			}
		}
	})().catch(error => {
...
// Dynamic import keeps Camoufox's optional native dependencies out of ordinary browser startup.
