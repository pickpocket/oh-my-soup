 ...
 * relay served by `oms browser-relay` (sibling `server.ts`/`bridge.ts`) plus
 * its companion extension (`packages/browser-relay`, installed via
 * `oms browser-relay install`). The relay impersonates Chrome's CDP discovery
...
/** Browser kind selecting the oms browser relay. */
export interface RelayKind {
	kind: "relay";
	cdpUrl: string;
}

/** Default endpoint of the `oms-browser-relay` CLI. */
