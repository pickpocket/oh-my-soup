...
export interface ToolSession {
	...
	/** Owning journal and its atomic notes mutation primitives. */
	sessionManager?: Pick<
		SessionManager,
		| "appendCustomEntry"
	...
	settings: Settings;
	/** Snapshot used by eval's `compact.status()` bridge. */
	getCompactionStatus?: () => {
		tokens: number | null;
	...
	/** Schedule an explicit compaction after the current eval cell settles. */
