...
#
# `__oms_emit_referenced_exports` reads captured function bodies from stdin,
...
__oms_sq_quote() {
	...
	__oms_qout=
	__oms_sq=\'
	while case "$__oms_qbuf" in *$__oms_sq*) true ;; *) false ;; esac; do
		__oms_qout=$__oms_qout${__oms_qbuf%%"$__oms_sq"*}"'\\''"
		__oms_qbuf=${__oms_qbuf#*"$__oms_sq"}
	done
	__oms_qout=$__oms_qout$__oms_qbuf
	printf "'%s'" "$__oms_qout"
}
...
__oms_emit_export_for() {
	...
	__oms_xv=
	eval "__oms_xv=\"\${$1}\"" 2>/dev/null || return
	printf 'export %s=%s\n' "$1" "$(__oms_sq_quote "$__oms_xv")"
}
...
__oms_emit_referenced_exports() {
	grep -oE '\$\{?[A-Za-z_][A-Za-z0-9_]*' \
		| sed -E 's/^\$\{?//' \
		| sort -u \
		| while IFS= read -r __oms_name; do
			__oms_emit_export_for "$__oms_name"
		done
}
