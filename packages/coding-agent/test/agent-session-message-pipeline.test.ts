...
describe("AgentSession message pipeline", () => {
	...
	it("preserves append-only prefixes in subagent sessions when context handlers rewrite prior turns", async () => {
		...
		const model = buildModel({
			...
			input: ["text"],
			cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
			// Large declared window: the important-notes headroom reminder fires
			// once prompt+tool overhead crosses 80% of a tiny window, which would
			// inject a developer message into these deterministic prefix checks.
