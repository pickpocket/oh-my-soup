...
describe("auth-broker import (CLIProxyAPI)", () => {
	...
	beforeEach(async () => {
		originalAgentDir = process.env.OMS_AGENT_DIR;
		...
		delete process.env.OMS_AUTH_BROKER_URL;
		delete process.env.OMS_AUTH_BROKER_TOKEN;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-import-agent-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-import-cliproxy-"));
		setAgentDir(agentDir);
	});

	afterEach(async () => {
		process.stdout.write = ORIGINAL_STDOUT_WRITE;
		if (originalAgentDir === undefined) delete process.env.OMS_AGENT_DIR;
		else process.env.OMS_AGENT_DIR = originalAgentDir;
	...
	test("imports a directory of CLIProxyAPI JSONs and maps types to oms providers", async () => {
...
describe("auth-broker import (broker-routed)", () => {
	...
	beforeEach(async () => {
		savedEnv.OMS_AUTH_BROKER_URL = process.env.OMS_AUTH_BROKER_URL;
		savedEnv.OMS_AUTH_BROKER_TOKEN = process.env.OMS_AUTH_BROKER_TOKEN;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-import-client-"));
		brokerAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-import-broker-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-import-cliproxy-broker-"));
		setAgentDir(agentDir);
