 ...
 *
 * One-shot CLIs (`oms bench`, dry-balance) build a bare `ModelRegistry` that
 ...
 * `oms bench <provider>/<model>` failed with "Model not found".
 *
 ...
 * passed explicitly so the test never touches the developer's real `~/.oms`.
 */
