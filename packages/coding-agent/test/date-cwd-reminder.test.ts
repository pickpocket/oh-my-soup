...
import type { Api, Context, Message, Model, ModelSpec } from "@oh-my-soup/pi-ai";
...
import { DateCwdReminderInjector, renderDateCwdReminder } from "@oh-my-soup/pi-coding-agent/session/date-cwd-reminder";
import { SessionManager } from "@oh-my-soup/pi-coding-agent/session/session-manager";
import { formatLocalCalendarDate } from "@oh-my-soup/pi-tui/chrome/local-date";
import { normalizePromptPath } from "@oh-my-soup/pi-coding-agent/utils/prompt-path";
...
describe("date-cwd-reminder", () => {
	...
	describe("renderDateCwdReminder", () => {
		it("renders a system-reminder block carrying the date and cwd with a do-not-repeat instruction", () => {
			const reminder = renderDateCwdReminder("2026-08-14", "C:/work/oms");
			...
			expect(reminder).toContain("C:/work/oms");
	...
	describe("DateCwdReminderInjector", () => {
		it("injects the first reminder without mutating the context", () => {
			...
			const out = injector.transform(context, "2026-08-14", "/work/oms");
			...
			expect(out.messages[0]).toEqual({
				role: "user",
				content: `${renderDateCwdReminder("2026-08-14", "/work/oms")}\n\nhello`,
				timestamp: 1,
			});
		...
		it("prepends a text part before image parts", () => {
			...
			const out = new DateCwdReminderInjector().transform(context, "2026-08-14", "/work/oms");

			expect(out.messages[0]?.content).toEqual([
				{ type: "text", text: renderDateCwdReminder("2026-08-14", "/work/oms") },
		...
		it("reuses injected message objects on provider request replay", () => {
			...
			const first = injector.transform(context, "2026-08-14", "/work/oms");
			const replay = injector.transform({ ...context, messages: [...context.messages] }, "2026-08-14", "/work/oms");
