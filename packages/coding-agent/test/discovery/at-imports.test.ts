...
import { expandAtImports, MAX_AT_IMPORT_DEPTH } from "@oh-my-soup/pi-coding-agent/discovery/at-imports";
...
describe("expandAtImports", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "oms-at-import-"));
	});
	...
	test("resolves ~/path against the home override", async () => {
		const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "oms-at-home-"));
		try {
