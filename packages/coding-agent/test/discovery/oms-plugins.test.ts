...
import { getCapability, loadCapability } from "@oh-my-soup/pi-coding-agent/capability";
...
test("project config.yml#extensions surfaces every sub-directory (#9768)", async () => {
	writeFile(path.join(project, ".oms", "config.yml"), `extensions:\n  - "${ext}"\n`);
	await expectExtensionSubDirectoriesLoaded(ctx());
});
...
test("user config.yaml#extensions feeds sub-discovery", async () => {
	...
	writeFile(path.join(home, ".oms", "agent", "config.yaml"), `extensions:\n  - "${ext}"\n`);
...
test("project config.yml#extensions replaces lower-precedence configured roots", async () => {
	...
	writeFile(path.join(home, ".oms", "agent", "config.yml"), `extensions:\n  - "${userExt}"\n`);
	writeFile(path.join(project, ".oms", "settings.json"), JSON.stringify({ extensions: [projectSettingsExt] }));
	writeFile(path.join(project, ".oms", "config.yml"), `extensions:\n  - "${projectConfigExt}"\n`);
...
test("user YAML config suppresses its legacy settings.json migration source", async () => {
	...
	writeFile(path.join(home, ".oms", "agent", "settings.json"), JSON.stringify({ extensions: [legacyExt] }));
	writeFile(path.join(home, ".oms", "agent", "config.yml"), "theme:\n  dark: default\n");
...
test("concurrent scopes snapshot their own effective extensions (no cross-session leak)", async () => {
	...
	const secondEntered = Promise.withResolvers<void>();
	const [firstRoots, secondRoots] = await Promise.all([
		withOmsExtensionRootScope([], "merge", async () => {
...
test("explicit-only mode drops the configured lane and installed roots (#9769)", async () => {
	...
	const mergeRoots = await listOmsExtensionRoots({
		...ctx(),
		extensionRoots: {
	...
	const explicitOnlyRoots = await listOmsExtensionRoots({
		...ctx(),
		extensionRoots: {
			explicit: [explicitExt],
			mode: "explicit-only",
...
test("configured lane takes its level from the authority's configuredLevel, not the disk scan (#9769)", async () => {
	// A project provider Settings can't see on the `.oms` disk scan (e.g.
	...
	// Nothing on `.oms` disk configures it — the old deepEquals scan would label it `user`.

	const asProject = await listOmsExtensionRoots({
		...ctx(),
	...
	const asUser = await listOmsExtensionRoots({
		...ctx(),
...
test("scopeless reload with session roots equals the construction-time scoped load (#9769 invariant)", async () => {
	...
	// session, listOmsExtensionRoots outside the construction scope with
	...
	const installed = path.join(home, ".oms", "plugins", "node_modules", "invariant-installed");
	...
	writeFile(
		path.join(home, ".oms", "plugins", "package.json"),
		JSON.stringify({ name: "oms-plugins", dependencies: { "invariant-installed": "1.0.0" } }),
	);
	...
	for (const mode of ["merge", "explicit-only"] as const) {
		for (const configured of [[configuredExt], []]) {
			for (const configuredLevel of ["user", "project"] as const) {
				...
				const scoped = await withOmsExtensionRootScope(roots.explicit, roots.mode, () => {
					setInvocationConfiguredExtensions(roots.configured, roots.configuredLevel);
					return listOmsExtensionRoots(ctx());
				});
				const reloaded = await listOmsExtensionRoots({ ...ctx(), extensionRoots: roots });
...
test(".mcp.json expands environment placeholders recursively", async () => {
	const variables = {
		OMS_PLUGIN_COMMAND: "expanded-command",
		OMS_PLUGIN_ARG: "expanded-arg",
		OMS_PLUGIN_ENV: "expanded-env",
		OMS_PLUGIN_CWD: path.join(tempDir, "expanded-cwd"),
		OMS_PLUGIN_URL: "https://mcp.example.test",
		OMS_PLUGIN_HEADER: "expanded-header",
		OMS_PLUGIN_CLIENT_ID: "expanded-client-id",
	};
	...
	try {
		writeFile(
			...
			JSON.stringify({
				mcpServers: {
					stdio: {
						command: placeholder("OMS_PLUGIN_COMMAND"),
						args: [placeholder("OMS_PLUGIN_ARG")],
						env: { TOKEN: placeholder("OMS_PLUGIN_ENV") },
						cwd: placeholder("OMS_PLUGIN_CWD"),
					},
					http: {
						type: "http",
						url: placeholder("OMS_PLUGIN_URL"),
						headers: { Authorization: `Bearer ${placeholder("OMS_PLUGIN_HEADER")}` },
						oauth: { clientId: placeholder("OMS_PLUGIN_CLIENT_ID") },
					},
				},
			}),
		);
		...
		expect(stdio).toMatchObject({
			command: variables.OMS_PLUGIN_COMMAND,
			args: [variables.OMS_PLUGIN_ARG],
			env: { TOKEN: variables.OMS_PLUGIN_ENV },
			cwd: variables.OMS_PLUGIN_CWD,
		});
		expect(http).toMatchObject({
			url: variables.OMS_PLUGIN_URL,
			headers: { Authorization: `Bearer ${variables.OMS_PLUGIN_HEADER}` },
			oauth: { clientId: variables.OMS_PLUGIN_CLIENT_ID },
		});
	} finally {
