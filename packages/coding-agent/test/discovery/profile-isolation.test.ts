/**
 * Regression: OMS-native user-level config discovery must follow the active
 * profile. A profile relocates the agent directory to ~/.oms/profiles/<name>/agent;
 * the native provider used to read user config (commands, skills, rules, etc.)
 * from the literal home (~/.oms/agent) via `ctx.home`, leaking the default
 ...
 * (ctx.home + ".oms/agent") points at the tempdir decoys below; without the fix
...
import { type Skill, skillCapability } from "@oh-my-soup/pi-coding-agent/capability/skill";
...
describe("native user-level config discovery follows the active profile", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "oms-profile-iso-home-"));
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-profile-iso-project-"));
		profileAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-profile-iso-agent-"));
