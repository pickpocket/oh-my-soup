/**
 * Discovery integration tests for OMS plugin registry reading.
 *
 * NOTE: listClaudePluginRoots() lives in discovery/helpers.ts which imports
 * @oh-my-soup/pi-natives (native Rust addon via glob). We cannot call it here.
 *
 * Instead these tests validate the structural contract that listClaudePluginRoots
 * depends on:
 *   1. OMS registry lives at path.join(home, ".oms", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".oms")
...
// Kept here to avoid importing helpers.ts (which pulls in @oh-my-soup/pi-natives).
...
// Matches getConfigDirName() — single source of truth is in @oh-my-soup/pi-utils,
// but we know the value is ".oms" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const OMS_CONFIG_DIR = ".oms";
...
let tmpHome: string;
/** ~/.oms/plugins/installed_plugins.json inside tmpHome */
let ompRegistryPath: string;

beforeEach(() => {
	tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "oms-discovery-test-"));
	ompRegistryPath = path.join(tmpHome, OMS_CONFIG_DIR, "plugins", "installed_plugins.json");
...
describe("OMS registry path contract", () => {
	it("OMS registry lives at home/.oms/plugins/installed_plugins.json", () => {
		// This is the path that listClaudePluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".oms", "plugins", "installed_plugins.json");
...
describe("OMS registry format compatibility with Claude parser", () => {
...
describe("OMS registry round-trip", () => {
...
//
// listClaudePluginRoots must replace Claude entries with OMS entries when the same
...
describe("OMS precedence contract (registry structure)", () => {
	it("same plugin ID in both registries — OMS entry has required fields for deduplication", () => {
		// The replacement logic: roots.filter(r => r.id !== pluginId) keyed by id.
		// OMS entries must have installPath so they can be added to roots[].
		const id = buildPluginId("shared-plugin", "common-mkt");
		const ompEntry = makeEntry("/oms/cached/path");

		// OMS registry entry has installPath (required by listClaudePluginRoots)
