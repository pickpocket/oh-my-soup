...
describe("plugin config", () => {
	...
	test("updates user marketplace runtime features despite a project shadow while list stays duplicate-free", async () => {
		const pluginName = "oms-featureful";
		const installPath = path.join(pluginsDir, "cache", pluginName);
		const pluginPath = path.join(pluginsDir, "node_modules", pluginName);
		await Bun.write(
			path.join(installPath, "package.json"),
			JSON.stringify({
				name: pluginName,
				version: "1.0.0",
				oms: { version: "1.0.0", features: { review: { description: "Review changes" } } },
			}),
		);
		await fs.mkdir(path.dirname(pluginPath), { recursive: true });
		await fs.symlink(installPath, pluginPath, "dir");
		...
		const projectPluginsDir = path.join(tmpRoot, ".oms", "plugins");
		...
		await Bun.write(
			...
			JSON.stringify({
				name: pluginName,
				version: "2.0.0",
				oms: { version: "2.0.0", features: { projectOnly: { description: "Project-only feature" } } },
			}),
		);
		await fs.mkdir(path.dirname(projectPluginPath), { recursive: true });
		await fs.symlink(projectInstallPath, projectPluginPath, "dir");
		await Bun.write(
			path.join(projectPluginsDir, "oms-plugins.lock.json"),
			JSON.stringify({
		...
		).rejects.toThrow(/Unknown feature "projectOnly" in oms-featureful/);
	...
	async function writeManifest(dir: string, manifest: Record<string, unknown>): Promise<void> {
		await Bun.write(
			...
			JSON.stringify({ name: "oms-commit", version: "2.0.0", ...manifest }),
		);
	}

	async function installProjectMarketplacePlugin(schemaDefault: string, enabled = true): Promise<string> {
		const installPath = path.join(tmpRoot, "cache", `oms-commit-project-${schemaDefault}`);
		...
		const projectRoot = path.join(tmpRoot, ".oms", "plugins");
		await fs.mkdir(path.join(projectRoot, "node_modules"), { recursive: true });
		await fs.symlink(installPath, path.join(projectRoot, "node_modules", "oms-commit"), "dir");
		await Bun.write(
			path.join(projectRoot, "oms-plugins.lock.json"),
			JSON.stringify({
				plugins: { "oms-commit": { version: "2.0.0", enabledFeatures: null, enabled } },
				settings: {},
			}),
		);
		return installPath;
	...
	test("resolves a project-scoped marketplace plugin absent from the user root", async () => {
		...
		const manager = new PluginManager(tmpRoot);
		expect(await manager.list()).toEqual([]);
		expect((await manager.getPlugin("oms-commit"))?.manifest.settings?.splitMode?.default).toBe("auto");
	});
	...
	test("prefers the active project plugin over a same-named user install", async () => {
		...
		const userPkg = path.join(pluginsDir, "node_modules", "oms-commit");
		...
		await Bun.write(
			lockfile,
			JSON.stringify({
				plugins: { "oms-commit": { version: "1.0.0", enabledFeatures: null, enabled: true } },
				settings: {},
			}),
		);
	...
	test("falls back to an enabled user plugin when the project copy is disabled", async () => {
		...
		const manager = new PluginManager(tmpRoot);
		expect((await manager.getPlugin("oms-commit"))?.manifest.settings?.splitMode?.default).toBe("manual");
	});
});
