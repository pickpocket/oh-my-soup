...
describe("plugin extension discovery", () => {
	...
	beforeEach(() => {
		...
		//  - mock os.homedir() so configRoot = `<tempHome>/.oms` (the previous
		...
		//    `$XDG_DATA_HOME/oms` over the home config root when that dir exists, so an
		...
		// ~/.oms must fail loudly here instead of clobbering the developer's plugins.
	...
	it("resolves a sub-extension directory via its own package.json manifest over a decoy index", async () => {
		...
		fs.writeFileSync(
			...
			JSON.stringify({ name: "feature-ext", version: "1.0.0", oms: { extensions: ["./dist/real-ext.ts"] } }),
		);
		fs.writeFileSync(
			realEntry,
			[
