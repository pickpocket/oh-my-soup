/**
 * `oms read <image>?q=<question>` delegates to a vision model, which requires a
 ...
 * resolution (issue #11338). This drives the real `oms read` command in an
...
describe("oms read <image>?q=", () => {
	it("resolves a vision model instead of failing with the registry guard", async () => {
		...
		try {
			...
			fs.mkdirSync(path.join(project, ".oms"), { recursive: true });
			...
			fs.writeFileSync(path.join(project, ".oms", "config.yml"), "images:\n  questionTimeoutMs: 3000\n");
