...
describe("skillful setting and /skillful session toggle", () => {
	...
	beforeEach(() => {
		...
		fs.mkdirSync(path.join(tempDir, ".oms", "skills", "test-skill"), { recursive: true });
		fs.writeFileSync(
			path.join(tempDir, ".oms", "skills", "test-skill", "SKILL.md"),
	...
	it("announces URI syntax without catalog rows for hidden-only skills mid-session", async () => {
		const skillDir = path.join(tempDir, ".oms", "skills", "test-skill");
