...
import { BUILTIN_SLASH_COMMANDS } from "@oh-my-soup/pi-coding-agent/slash-commands/builtin-registry";
...
describe("/move directory completion", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-move-completion-"));
	...
	it("completes parent directory paths", async () => {
		...
		const siblingName = `oms-move-sibling-${path.basename(tempDir)}`;
