...
import * as connectionManager from "@oh-my-soup/pi-coding-agent/ssh/connection-manager";
import { removeWithRetries } from "@oh-my-soup/pi-utils";

async function withLooseKey<T>(run: (keyPath: string) => Promise<T>): Promise<T> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-ssh-key-"));
...
describe("buildRemoteCommand", () => {
	...
	it("still rejects directory identity paths on Windows args", async () => {
		const keyPath = await fs.mkdtemp(path.join(os.tmpdir(), "oms-ssh-key-"));
		try {
			await expect(
				connectionManager.buildRemoteCommand(
					{
