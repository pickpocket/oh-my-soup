...
describe("disposeTerminalTitleState", () => {
	...
	let prevWslMarkers: { distro?: string; interop?: string } = {};

	beforeEach(() => {
		vi.useFakeTimers();
		// WSL is a static-title host (isWsl reads live env), which would suppress
		// the spinner ticks these tests observe. Clear the interop markers so the
		// suite behaves identically under a WSL runner and plain Linux CI.
		prevWslMarkers = { distro: process.env.WSL_DISTRO_NAME, interop: process.env.WSL_INTEROP };
	...
	afterEach(() => {
		...
		if (prevWslMarkers.distro !== undefined) process.env.WSL_DISTRO_NAME = prevWslMarkers.distro;
		if (prevWslMarkers.interop !== undefined) process.env.WSL_INTEROP = prevWslMarkers.interop;
		vi.useRealTimers();
	});
