...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@oh-my-soup/pi-utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.oms/`), plus
//    the legacy non-XDG `~/.oms/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.oms/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/oms/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.oms` when
...
// symlinks `~/.oms/agent/extensions/foo.ts` to a dotfiles repo would see the
