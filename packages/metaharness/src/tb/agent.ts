...
/** Build and cache self-contained oms binaries for the selected guest architectures. */
export async function prepareAgentBinaries(opts: {
	...
	for (const arch of arches) {
		cached[arch] = path.join(opts.cacheDir, `oms-linux-${arch}-${manifest.version}`);
	}
	...
	if (missing.length > 0) {
		...
		for (const arch of missing) {
			const source = path.join(CODING_AGENT_DIR, "binaries", `oms-linux-${arch}`);
...
/** Install oms and gateway-only configuration into one running trial microVM. */
export async function installAgent(vm: TrialVm, binaries: AgentBinaries, gateway: GatewayConfig): Promise<string> {
	...
	if (!binary) throw new Error(`No oms binary available for guest architecture ${vm.arch}`);

	const entrypoint = "/opt/oms/oms";
	const mkdir = await vm.exec("mkdir -p /opt/oms");
	if (mkdir.exitCode !== 0) throw new Error(`Could not create /opt/oms: ${mkdir.stderr.trim()}`);
	await vm.copyTo(binary, entrypoint);
	const chmod = await vm.exec(`chmod 755 ${shellQuote(entrypoint)}`);
	if (chmod.exitCode !== 0) throw new Error(`Could not make oms executable: ${chmod.stderr.trim()}`);
	const warmup = await vm.exec(`${shellQuote(entrypoint)} --version`);
	if (warmup.exitCode !== 0) throw new Error(`oms --version failed: ${warmup.stderr.trim()}`);
...
`;
	const writeConfig = await vm.exec(
		`mkdir -p "$HOME/.oms/agent"\ncat > "$HOME/.oms/agent/models.yml" <<'OMS_MODELS_EOF'\n${modelsYaml}OMS_MODELS_EOF\ncat > "$HOME/.oms/agent/config.yml" <<'OMS_CONFIG_EOF'\n${configYaml}OMS_CONFIG_EOF`,
	);
	if (writeConfig.exitCode !== 0) throw new Error(`Could not install oms configuration: ${writeConfig.stderr.trim()}`);
	return entrypoint;
}
