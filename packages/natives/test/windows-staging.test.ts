/**
 * Regression for the Windows `bun install -g` update path: when an `oms`
...
const winNodeModulesNativeDir = "C:\\Users\\Admin\\node_modules\\@oh-my-soup\\pi-natives\\native";
const winWorkspaceNativeDir = "C:\\Users\\Admin\\dev\\oh-my-soup\\packages\\natives\\native";
...
describe("windows native addon staging", () => {
	...
	it("prepends versionedDir candidates ahead of node_modules when staging on Windows", () => {
		const versionedDir = "C:\\Users\\Admin\\.oms\\natives\\15.0.1";
		const userDataDir = "C:\\Users\\Admin\\AppData\\Local\\oms";
	...
	it("classifies only Windows node_modules paths case-insensitively", () => {
		...
		try {
			const workspace = initLoaderContext({
				platform: "linux",
				isCompiledBinary: false,
				nativeDir: "/tmp/oh-my-soup/packages/natives/native",
				leafPackageDir,
			});
	...
	it("falls back to the node_modules-only candidate list when staging is off", () => {
		...
		const versionedDir = "/home/u/.oms/natives/15.0.1";
		const candidates = resolveLoaderCandidates({
	...
	it("removes only older version directories after the current native version loads", async () => {
		const nativesDir = await fs.mkdtemp(path.join(os.tmpdir(), "oms-natives-cache-"));
