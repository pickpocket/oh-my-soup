...
```sh
npm install @oh-my-soup/omstype
# or
bun add @oh-my-soup/omstype
```
...
Failed validation returns `OmsErrors`; each entry exposes `code`, `path`,
...
TypeBox-style and Zod-style builders produce native omstype schemas:
...
`@oh-my-soup/omstype/ark` provides the repository's ArkType compatibility facade
...
```sh
bun packages/omstype/bench/bench.ts
```
...
candidate was than omstype in this run. Results vary with hardware, runtime,
