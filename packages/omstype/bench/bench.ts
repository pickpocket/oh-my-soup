...
import { omstypeCandidate } from "./candidates/omstype";
import { typeboxCandidate } from "./candidates/typebox";
import { FIXTURES, generateUniqueDefs } from "./fixtures";
import type { Def } from "./ir";

const registry: Candidate[] = [omstypeCandidate, arktypeCandidate, typeboxCandidate];
...
function report(title: string, rows: Row[]): void {
	const baseline = rows.find(row => row.candidate === "omstype");
	console.log(`\n== ${title} ==`);
	for (const row of rows) {
		let relative = "";
		if (baseline && row.candidate !== "omstype") {
