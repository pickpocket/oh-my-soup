 ...
 * `result instanceof type.errors` / `instanceof OmsErrors`, lazy `.summary`,
...
export const MISSING: unique symbol = Symbol("omstype.missing");
...
export class OmsErrors implements Iterable<OmsError> {
	#path: StoredPath;
	#expected: string;
	#data: unknown;
	#entry: OmsError | undefined;
	#entries: OmsError[] | undefined;
	#config: ErrorConfig | undefined;
	#separator = "\n";
	...
	static single(path: PropertyKey[], expected: string, data: unknown, config?: ErrorConfig): OmsErrors {
		return new OmsErrors(path, expected, data, config);
	}

	#getEntry(): OmsError {
		if (this.#entry) return this.#entry;
		const path = this.#path === undefined ? [] : Array.isArray(this.#path) ? [...this.#path] : [this.#path];
		const entry = new OmsError(path, this.#expected, this.#data, this.#config);
		this.#entry = entry;
		return entry;
	}

	/** Append all failures from `other`, preserving traversal order. */
	append(other: OmsErrors): this {
	...
	get byPath(): Readonly<Record<string, OmsError>> {
		const result: Record<string, OmsError> = {};
		for (const entry of this) result[entry.path.map(String).join(".")] = entry;
		return result;
	}

	map<result>(fn: (error: OmsError, index: number, errors: OmsErrors) => result): result[] {
		const result: result[] = [];
		let index = 0;
	...
	filter(fn: (error: OmsError, index: number, errors: OmsErrors) => unknown): OmsError[] {
		const result: OmsError[] = [];
		let index = 0;
	...
	*[Symbol.iterator](): IterableIterator<OmsError> {
		if (this.#entries) {
			yield* this.#entries;
		} else {
			yield this.#getEntry();
...
export class TraversalError extends Error {
	constructor(readonly errors: OmsErrors) {
 ...
 * validation failures, which are returned as {@link OmsErrors}.
 */
export class OmsTypeError extends Error {
	constructor(message: string) {
		super(message);
		this.name = "OmsTypeError";
	}
}
