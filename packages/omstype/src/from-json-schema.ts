/**
 * JSON Schema → omstype importer.
 *
 * Rebuilds a callable schema from JSON Schema documents — the inverse of
 * `Type.toJsonSchema()` for the dialect omstype (and its Zod/TypeBox
 ...
 * omstype IR and passes the rest through.
 */
...
class Importer {
	...
	resolveRef(ref: string): IR {
		...
		if (ref === "#") {
			...
			if (defsMatch === null) throw new OmsTypeError(`unsupported $ref: ${ref}`);
			const defs = this.#root[defsMatch[1]];
			target = typeof defs === "object" && defs !== null ? (defs as JsonSchema)[defsMatch[2]] : undefined;
			if (target === undefined) throw new OmsTypeError(`unresolved $ref: ${ref}`);
		}
	...
	lower(schema: unknown): IR {
		...
		if (typeof schema !== "object" || schema === null) {
			throw new OmsTypeError("JSON Schema nodes must be booleans or objects");
		}
	...
	#lowerTyped(node: JsonSchema, kind: string): IR {
		switch (kind) {
			...
			default:
				throw new OmsTypeError(`unsupported JSON Schema type: ${kind}`);
		}
	}
...
/**
 * Build a callable omstype schema from a JSON Schema document.
 *
 * # Errors
 * Throws {@link OmsTypeError} on malformed nodes, unresolvable `$ref`s, or
 * types omstype cannot represent.
 */
