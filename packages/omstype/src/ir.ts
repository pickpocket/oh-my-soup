...
import { type ErrorConfig, OmsErrors, OmsTypeError } from "./errors";
import { keywordIR, patternIR, templateIR } from "./keywords";

/** Brand carried by `Type` instances so the parser can embed them in defs. */
export const IR_BRAND: unique symbol = Symbol("omstype.schema");

const kMorph: unique symbol = Symbol("omstype.hasMorph");
const kMorphOwner: unique symbol = Symbol("omstype.hasMorphOwner");
const kAlias: unique symbol = Symbol("omstype.hasAlias");
const kAliasOwner: unique symbol = Symbol("omstype.hasAliasOwner");
const kSimple: unique symbol = Symbol("omstype.simple");
const kSimpleOwner: unique symbol = Symbol("omstype.simpleOwner");
...
export type IR = IRAnalysis &
	(
		...
		| {
				k: "refine";
				base: IR;
				pred: (value: unknown) => boolean | OmsErrors;
				expected: string;
...
function tokenize(src: string): Tok[] {
	...
	while (i < n) {
		...
		if (c === "d" && (src[i + 1] === "'" || src[i + 1] === '"')) {
			...
			if (end < 0) throw new OmsTypeError(`unterminated date literal in "${src}"`);
			const source = src.slice(i + 2, end).trim();
			const value = /^\d+$/.test(source) ? new Date(Number(source)) : new Date(source);
			if (Number.isNaN(value.valueOf())) throw new OmsTypeError(`invalid date literal in "${src}"`);
		...
		if (c === "'" || c === '"') {
			...
			if (j >= n) throw new OmsTypeError(`unterminated string literal in "${src}"`);
		...
		if (c === "/") {
			...
			if (j >= n) throw new OmsTypeError(`unterminated regular expression in "${src}"`);
			let end = j + 1;
			...
			try {
				...
				throw new OmsTypeError(`invalid regular expression "${src.slice(i, end)}"`);
			}
			i = end;
			continue;
		}
		if ((c >= "0" && c <= "9") || (c === "-" && i + 1 < n && src[i + 1] >= "0" && src[i + 1] <= "9")) {
			...
			if (/^-?(?:0|[1-9]\d*)n$/.test(raw) && raw !== "-0n") {
				...
				if (!valid) throw new OmsTypeError(`Malformed number literal '${raw}'`);
		...
		throw new OmsTypeError(`unexpected character '${c}' in "${src}"`);
	}
	return toks;
}
...
class StrParser {
	...
	#next(): Tok {
		...
		if (!t) throw new OmsTypeError(`unexpected end of definition "${this.#src}"`);
		return t;
	}
	...
	parseTop(): ParsedTop {
		...
		if (this.#eatOp("=")) {
			...
			else throw new OmsTypeError(`unsupported default literal in "${this.#src}"`);
			hasDefault = true;
		}
	...
	#expectEnd(): void {
		if (this.#pos < this.#toks.length) {
			throw new OmsTypeError(`trailing tokens in definition "${this.#src}"`);
		}
	}
		...
		if (literal && typeof literal.v === "number") {
			for (const member of members) {
				if (
					...
					throw new OmsTypeError("literal is excluded by intersection");
				}
			}
			return literal;
		}
	...
	parseBounded(): IR {
		...
		if ((t?.t === "num" || t?.t === "date") && t1?.t === "op" && (t1.v === "<" || t1.v === "<=")) {
			...
			const t2 = this.#peek();
			if (!(t2?.t === "op" && CMP[t2.v])) {
				throw new OmsTypeError(`left bound requires a corresponding right bound in "${this.#src}"`);
			}
			if (t2.v === ">" || t2.v === ">=") {
				throw new OmsTypeError(`right bound must use < or <= in "${this.#src}"`);
			}
			this.#pos++;
			const hi = this.#next();
			if (hi.t !== "num" && hi.t !== "date") {
				throw new OmsTypeError(`expected bound after comparator in "${this.#src}"`);
			}
		...
		if (t2?.t === "op" && t2.v === "==") {
			...
			if (limit.t !== "num" && limit.t !== "bigint" && limit.t !== "date") {
				throw new OmsTypeError(`expected literal after == in "${this.#src}"`);
			}
	...
	#eatDivisor(node: IR): IR {
		...
		if (divisor.t !== "num") throw new OmsTypeError(`expected number after % in "${this.#src}"`);
		if (node.k !== "number") throw new OmsTypeError(`% requires number in "${this.#src}"`);
		if (!Number.isFinite(divisor.v) || !Number.isInteger(divisor.v) || divisor.v === 0)
			throw new OmsTypeError(`divisor must be a non-zero integer in "${this.#src}"`);
	...
	#eatArraySuffixes(node: IR): IR {
		for (;;) {
			...
			this.#pos++;
			if (!this.#eatOp("]")) throw new OmsTypeError(`expected ']' in "${this.#src}"`);
	...
	parsePostfix(): IR {
		...
		for (;;) {
			...
			if (t?.t === "op" && t.v === "#") {
				...
				if (name.t !== "id") throw new OmsTypeError(`expected brand name after # in "${this.#src}"`);
				continue;
			}
			break;
		}
	...
	#parseGenericArguments(): IR[] {
		...
		for (;;) {
			...
			if (this.#eatOp(">")) return arguments_;
			if (!this.#eatOp(",")) throw new OmsTypeError(`expected ',' or '>' in "${this.#src}"`);
			const next = this.#peek();
			if (next?.t === "op" && (next.v === "," || next.v === ">")) {
				throw new OmsTypeError(`generic arguments cannot be empty in "${this.#src}"`);
			}
		}
	}
		...
		if (t.t === "id") {
			if (t.v === "keyof") {
				try {
					...
					if (error instanceof OmsTypeError) throw new OmsTypeError("keyof operand must be an object");
					throw error;
				}
			}
			...
			if (t.v === "Record" && this.#peek()?.t === "op" && this.#peek()?.v === "<") {
				...
				if (arguments_.length !== 2) throw new OmsTypeError("Record requires two arguments");
			...
			if (this.#peek()?.t === "op" && this.#peek()?.v === "<" && this.#resolve?.hasGeneric?.(t.v)) {
				...
				if (!instantiated) throw new OmsTypeError(`unknown generic "${t.v}" in "${this.#src}"`);
				return instantiated;
			}
			const scoped = this.#resolve?.(t.v);
			const make = KEYWORDS[t.v];
			const keyword = scoped ?? make?.() ?? keywordIR(t.v);
			if (!keyword) throw new OmsTypeError(`unknown keyword "${t.v}" in "${this.#src}"`);
			return keyword;
		}
		throw new OmsTypeError(`unexpected token in "${this.#src}"`);
	}
}
...
function resolveStructuralIR(ir: IR): IR {
	...
	while (ir.k === "alias") {
		if (seen.has(ir)) throw new OmsTypeError(`cannot structurally transform recursive alias "${ir.name}"`);
		seen.add(ir);
		ir = ir.resolve();
	}
	return ir;
...
function mergeObjectIR(left: IR, right: IR): IR {
	...
	if (left.k !== "object" || right.k !== "object") {
		throw new OmsTypeError("Merge requires object arguments");
	}
...
function parseGeneric(src: string, resolve?: AliasResolver): IR | undefined {
	...
	if ((generic.name === "Partial" || generic.name === "Required") && generic.args.length === 1) {
		...
		if (object.k !== "object") throw new OmsTypeError(`${generic.name} requires an object`);
...
function parseRegexExec(src: string): IR | undefined {
	...
	if (end < 2) throw new OmsTypeError(`unterminated regular expression in "${src}"`);
	let regex: RegExp;
	try {
		regex = new RegExp(src.slice(2, end), src.slice(end + 1));
	} catch {
		throw new OmsTypeError(`invalid regular expression "${src.slice(1)}"`);
	}
	return {
		k: "morph",
...
function parseStringDef(src: string, resolve?: AliasResolver): ParsedTop {
	...
	if (pipeIndex >= 0) {
		...
		if (input.length === 0 || output.length === 0) {
			throw new OmsTypeError(`pipe expression requires operands in "${src}"`);
		}
		const parsedInput = parseStringDef(input, resolve);
		if (parsedInput.hasDefault) {
			throw new OmsTypeError(`unexpected pipe expression after default in "${src}"`);
		}
...
function applyEquality(node: IR, value: number | bigint | Date, src: string): IR {
	...
	if (value instanceof Date) {
		if (!acceptsDate(node)) throw new OmsTypeError(`Date equality requires Date in "${src}"`);
	...
	if ((node.k === "string" || node.k === "array") && typeof value === "number") {
		if (!Number.isInteger(value) || value < 0) {
			throw new OmsTypeError(`exact length must be a non-negative integer in "${src}"`);
		}
		return { ...node, min: value, max: value };
	}
	throw new OmsTypeError(`equality literal is incompatible with ${node.k} in "${src}"`);
}
...
function applyBound(node: IR, op: string, value: number | Date, src: string): IR {
	...
	if (node.k === "union") {
		...
		if (kinds.size !== 1) throw new OmsTypeError(`cannot apply one bound to multiple bound kinds in "${src}"`);
	...
	if (value instanceof Date) {
		if (!acceptsDate(node)) throw new OmsTypeError(`date bound requires Date in "${src}"`);
	...
	if (node.k === "number") {
		...
		if (
			...
			throw new OmsTypeError(`numeric range is unsatisfiable in "${src}"`);
		}
		return bounded;
	}
	if (node.k === "string" || node.k === "array") {
		if (!Number.isInteger(value) || value < 0) {
			throw new OmsTypeError(`length bound must be a non-negative integer in "${src}"`);
		}
		const bounded = { ...node };
		switch (op) {
			case ">=":
		...
		if (
			...
			throw new OmsTypeError(`length range is unsatisfiable in "${src}"`);
		}
		return bounded;
	}
	throw new OmsTypeError(`cannot bound ${node.k} in "${src}"`);
}
...
function appendTupleItem(tuple: TupleIR, item: TupleItemIR): void {
	...
	if (item.hasDefault && tuple.prefix.some(prefixItem => prefixItem.opt && !prefixItem.hasDefault)) {
		throw new OmsTypeError("A defaultable element may not follow an optional element without a default");
	}
	if (hasOptionalPrefix(tuple) && !item.opt) {
		throw new OmsTypeError("required tuple elements cannot follow optional elements");
	}
...
function appendTuple(target: TupleIR, spread: TupleIR): void {
	if (target.variadic !== undefined && spread.variadic !== undefined) {
		throw new OmsTypeError("a tuple may have one spread followed by an array definition");
	}
...
function spreadAlternatives(spread: IR): TupleIR[] {
	...
	throw new OmsTypeError("tuple spread element must be an array");
}
...
export function keyOf(node: IR): IR {
	...
	if (node.k === "union") {
		...
		if (common.length === 0) throw new OmsTypeError("keyof operand must be an object");
		const members = common.map(value => ({ k: "lit", v: value }) satisfies IR);
		return members.length === 1 ? members[0] : { k: "union", members };
	}
	throw new OmsTypeError("keyof operand must be an object");
}
...
function parseArrayExpression(def: readonly unknown[], resolve?: AliasResolver): IR {
	...
	if (def[0] === "instanceof") {
		...
		for (let index = 1; index < def.length; index++) {
			...
			if (!isConstructor(ctor)) throw new OmsTypeError("instanceof operands must be constructors");
			members.push({
				k: "instance",
				ctor,
	...
	if (def.length === 3 && def[1] === "=>") {
		if (!isCallback(def[2])) throw new OmsTypeError("morph operator requires a function");
	...
	if (def.length === 3 && def[1] === ":") {
		if (!isCallback(def[2])) throw new OmsTypeError("narrow operator requires a predicate");
		...
		return {
			...
			pred: value => {
				...
				const error = (
					...
					const next = OmsErrors.single([...(detail.path ?? detail.relativePath ?? [])], detail.expected, value, {
						preserveActual: true,
				...
				return errors ?? (result instanceof OmsErrors ? result : result === true);
			},
			expected,
		};
	}
...
function indexKeyKind(
	...
	throw new OmsTypeError(`indexed key definition must resolve to a string or symbol (was ${expectedOf(key)})`);
}
...
function addObjectProp(props: PropIR[], spreadKeys: Set<PropertyKey> | undefined, prop: PropIR): void {
	...
	if (!spreadKeys.delete(prop.key)) throw new OmsTypeError(`duplicate object key ${String(prop.key)}`);
	props[previous] = prop;
}
...
function parseObjectDefinition(def: Record<PropertyKey, unknown>, resolve?: AliasResolver): IR {
	...
	for (const originalKey in def) {
		...
		if (originalKey === "+") {
			...
			else throw new OmsTypeError(`bad "+" value ${String(val)}`);
			continue;
		}
		if (originalKey === "...") {
			...
			if (spread === undefined) {
				throw new OmsTypeError(`object spread must resolve to an object literal (was ${expectedOf(parsed)})`);
			}
		...
		if (typeof originalKey === "string" && originalKey.startsWith("[") && originalKey.endsWith("]")) {
			...
			if (typeof val === "string") {
				...
				if (parsed.hasDefault) throw new OmsTypeError("index signatures cannot specify a default");
				value = parsed.ir;
			} else {
				if (Array.isArray(val) && val.length === 3 && val[1] === "=") {
					throw new OmsTypeError("index signatures cannot specify a default");
				}
			...
			if (regex === null) {
				...
				try {
					...
					throw new OmsTypeError(`invalid index signature pattern ${keyDefinition}`);
				}
			}
			const objectIndexes = indexes ?? { patterns: [] };
			indexes = objectIndexes;
		...
		if (key !== originalKey) {
			if (!spreadKeys?.has(key) && props.some(candidate => candidate.key === key)) {
				throw new OmsTypeError(`duplicate object key ${String(key)}`);
			}
		...
		if (opt && prop.hasDefault) throw new OmsTypeError(`optional key ${String(key)} cannot specify a default`);
...
export function parseDef(def: unknown, resolve?: AliasResolver): IR {
	if (typeof def === "string") {
		const parsed = parseStringDef(def, resolve);
		if (parsed.hasDefault) {
			throw new OmsTypeError("A default may only be specified for an object property or tuple element");
		}
		if (parsed.optional) {
			throw new OmsTypeError(`optional "?" marker is only valid on object property values`);
		}
		return parsed.ir;
	}
	...
	if (typeof def === "function") {
		...
		if (!isEmbedded(resolved)) {
			throw new OmsTypeError(`thunk must return a Type (was ${typeof resolved})`);
		}
		return embed(resolved);
	}
	if (isObjectDefinition(def)) return parseObjectDefinition(def, resolve);
	throw new OmsTypeError(`unsupported definition ${String(def)} (was ${typeof def})`);
}
