	...
	return Object.assign(schema, {
		...
		min(bound: number): ZodLikeSchema<Out> {
			...
			if (
				...
				if (!Number.isSafeInteger(bound) || bound < 0)
					throw new OmsTypeError("min length must be a nonnegative safe integer");
		...
		max(bound: number): ZodLikeSchema<Out> {
			...
			if (
				...
				if (!Number.isSafeInteger(bound) || bound < 0)
					throw new OmsTypeError("max length must be a nonnegative safe integer");
		...
		regex(expression: RegExp, message?: string): ZodLikeSchema<Out> {
			...
			if (!isStringLike) throw new OmsTypeError(`cannot apply regex to ${ir.k}`);
		...
		url(): ZodLikeSchema<Out> {
			...
			throw new OmsTypeError(`cannot apply url to ${ir.k}`);
		},
		optional(): ZodLikeSchema<Out | undefined> & OptionalSchemaMarker {
		...
		trim(): ZodLikeSchema<Out> {
			...
			if (ir.k === "morph" || schema.hasSteps) {
				let trimmed = schemaFromIR<Out>({
					...
					fn: v => {
						...
						if (typeof r !== "string") throw new OmsTypeError("trim requires a string output");
						return r.trim();
					},
					out: { k: "string" },
				});
			...
			throw new OmsTypeError(`cannot apply trim to ${ir.k}`);
		},
		transform<Next>(transformer: (value: Out) => Next): ZodLikeSchema<Next> {
			return decorate(
