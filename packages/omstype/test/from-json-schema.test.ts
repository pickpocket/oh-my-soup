...
import { fromJsonSchema, OmsErrors, type } from "../src";
...
describe("fromJsonSchema", () => {
	it("round-trips schemas emitted by toJsonSchema", () => {
		...
		expect(imported({ name: "job", mode: "delete", tags: [] })).toBeInstanceOf(OmsErrors);
		expect(imported({ name: "", mode: "read", tags: [] })).toBeInstanceOf(OmsErrors);
		expect(imported({ mode: "read", tags: [] })).toBeInstanceOf(OmsErrors);
		expect(imported({ name: "job", mode: "read", tags: [1] })).toBeInstanceOf(OmsErrors);
	});
	...
	it("supports enum, const, unions, intersections, and boolean schemas", () => {
		...
		expect(bounded(4)).toBeInstanceOf(OmsErrors);
		expect(fromJsonSchema(true)("anything")).toBe("anything");
		expect(fromJsonSchema(false)("anything")).toBeInstanceOf(OmsErrors);
	});
	...
	it("validates string formats and patterns", () => {
		...
		expect(email("nope")).toBeInstanceOf(OmsErrors);
		const patterned = fromJsonSchema({ type: "string", pattern: "^G-[A-Z0-9]+$", minLength: 3 });
		expect(patterned("G-X1")).toBe("G-X1");
		expect(patterned("bad")).toBeInstanceOf(OmsErrors);
		expect(patterned("G-")).toBeInstanceOf(OmsErrors);
	});
	...
	it("imports tuples with variadic tails and rejects undeclared keys on closed objects", () => {
		...
		expect(pair([1])).toBeInstanceOf(OmsErrors);
		...
		expect(closed({ a: "x", b: 1 })).toBeInstanceOf(OmsErrors);
	});
	...
	it("emits recursive scopes as $defs/$ref and round-trips them", () => {
		...
		expect(imported({ value: 1, next: { value: "two" } })).toBeInstanceOf(OmsErrors);
	...
	it("emits io-aware schemas: input keeps morph sources and optional defaults, output the reverse", () => {
		...
		expect(imported({ raw: 42 })).toBeInstanceOf(OmsErrors);
	});
	...
	it("io output uses the .to() target for piped schemas", () => {
		...
		expect(Uid(0)).toBeInstanceOf(OmsErrors);
