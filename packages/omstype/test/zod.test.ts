...
describe("zod-like trim and superRefine", () => {
	it("trims strings and applies constraints post-trim", () => {
		...
		expect(z.string().trim().regex(/^oms$/).parse("  oms  ")).toBe("oms");
		expect(z.string().trim().regex(/^oms$/).safeParse("  nope  ").success).toBe(false);
		...
		expect(z.object({ name: z.string().default(" oms ").trim() }).parse({})).toEqual({ name: "oms" });
	});
