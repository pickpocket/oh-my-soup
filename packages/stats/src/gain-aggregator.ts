...
import { getStatsDbPath, isEnoent, logger } from "@oh-my-soup/pi-utils";
...
const TEMP_PATH_RE = /(?:^|\/)(?:T|tmp|pi-bash-exec|oms-bash-exec|pi-bash-detach)(?:\/|$)|^\/var\/folders(?:\/|$)/;
 ...
 *
 * Rules are generic: oms internal wt paths are dropped; conventional worktree
...
export function normalizeProjectPath(p: string): string | null {
	...
	if (/\/\.oms\/wt\//u.test(clean)) return null;
