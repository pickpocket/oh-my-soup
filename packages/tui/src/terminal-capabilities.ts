...
/**
 * Resolve the image protocol for a non-forced runtime from the static terminal
 * profile, then apply host carve-outs. A `TERM` mux label identifies the
 * transport rather than its outer renderer, so it never establishes graphics
 * support by itself; users can opt in explicitly with `PI_FORCE_IMAGE_PROTOCOL`.
 *
 * `_isTTY` remains injectable for callers compiled against the prior helper
 * shape, but TTY state is intentionally not a capability signal.
 */
export function resolveImageProtocol(
	terminalId: TerminalId,
	env: NodeJS.ProcessEnv = Bun.env,
	_isTTY: boolean = process.stdout.isTTY === true,
): ImageProtocol | null {
	const imageProtocol =
		terminalId === "warp"
			? resolveWarpImageProtocol(process.platform, env)
			: getTerminalInfo(terminalId).imageProtocol;
	// Paseo's xterm.js renderer draws neither Kitty APC nor placeholders, so
	// its explicit embedder marker overrides any static terminal identity
	// (getpaseo/paseo#3850).
