 ...
 * - **Dev runs only.** In the compiled `oms` binary every module is pre-bundled
...
const MODULE_COMPLETE_KEY: symbol = Symbol.for("oms.moduleLoadComplete");
const MODULE_BODY_START_KEY: symbol = Symbol.for("oms.moduleBodyStart");
...
function bodyStartMarker(path: string): string {
	return `;globalThis[Symbol.for("oms.moduleBodyStart")]?.(${JSON.stringify(path)});\n`;
}

function completionMarker(path: string): string {
	return `\n;globalThis[Symbol.for("oms.moduleLoadComplete")]?.(${JSON.stringify(path)});\n`;
}
