/**
 * Set the OS-visible process name (`/proc/self/comm`) so `oms` shows up as
 * `oms` — not `bun` — in `ps`, `pgrep`, `killall`, `top`, `htop`, and systemd.
 *
 ...
 * `bun` and process-name-based tooling can't target oms (and `pkill bun` becomes
