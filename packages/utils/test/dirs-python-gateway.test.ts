...
import { getAgentDir, getConfigDirName, getPythonGatewayDir, setAgentDir } from "@oh-my-soup/pi-utils/dirs";
...
describe("python gateway directory", () => {
	...
	it("uses XDG state for the default agent profile", async () => {
		...
		process.env.PI_CONFIG_DIR = `.oms-test-${Snowflake.next()}`;
		...
		expect(getPythonGatewayDir()).toBe(path.join(process.env.XDG_STATE_HOME, "oms", "python-gateway"));
	});
