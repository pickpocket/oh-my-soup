...
own `releases` row and `.oms-session-<tag>` transcript.
...
Bot account needs **Write** on every repo in `ROBOMS_REPO_ALLOWLIST`. Use a
...
`ROBOMS_RELEASE_SENTINEL_ENABLED=false` by default because this workflow may
...
`ROBOMS_RELEASE_COMMIT_PREFIX` (default `chore: bump version to `), each
...
`ROBOMS_RELEASE_MAX_ROUNDS` (default 5) bounds automated repairs;
`ROBOMS_RELEASE_TASK_TIMEOUT_SECONDS` controls each round. Optional
`ROBOMS_RELEASE_MODEL` selects a release-only model or CSV pool and otherwise
falls back to `ROBOMS_MODEL`. Terminal states are intentionally silent on
...
docker compose exec roboms roboms status                   # dump issue + release tables
