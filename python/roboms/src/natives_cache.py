...
Ownership: cache root is provisioned ``root:oms 02770`` by ``entrypoint.sh``
so slot subprocesses (group ``oms``) can capture under setgid inheritance.
...
def _git_safe_directory_env(repo_dir: Path) -> dict[str, str]:
    ...
    ``roboms.sandbox._safe_directory_env`` but kept local to avoid a circular
        ...
        *,
        ...
        with _flock(self.lockfile(repo)):
            ...
            try:
                ...
                # the setgid `oms` model that lets other slots read it.
                # A copy creates a fresh inode owned by the orchestrator
                # (root) and inherits gid `oms` from the setgid 2770
                # cache root.
