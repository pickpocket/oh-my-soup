...
from roboms.dashboard import reset_index_cache, static_dir
from roboms.db import Database, close_database

# Minimum HTML the dashboard handler needs to render: `<title>` plus a script
# block carrying the `__ROBOMS_CONFIG__` sentinel. The real Vite-built bundle
# adds JS/CSS asset links; tests only care about the rendering contract.
_PLACEHOLDER_INDEX_HTML = (
    "<!doctype html>\n"
    '<html lang="en">\n'
    '  <head><meta charset="utf-8"><title>roboms</title></head>\n'
    "  <body>\n"
    '    <div id="app"></div>\n'
    '    <script id="roboms-config" type="application/json">__ROBOMS_CONFIG__</script>\n'
    "  </body>\n"
    "</html>\n"
)
...
def _baseline_env(tmp_path: Path) -> dict[str, str]:
    return {
        ...
        "ROBOMS_GH_PROXY_URL": "http://gh-proxy.invalid:8081",
        "ROBOMS_GH_PROXY_HMAC_KEY": "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "GITHUB_WEBHOOK_SECRET": "test-webhook-secret",
        "ROBOMS_BOT_LOGIN": "roboms-bot",
        "ROBOMS_GIT_AUTHOR_NAME": "roboms-bot",
        "ROBOMS_GIT_AUTHOR_EMAIL": "roboms-bot@example.invalid",
        "ROBOMS_REPO_ALLOWLIST": "octo/widget",
        "ROBOMS_MODEL": "anthropic/claude-sonnet-4-5",
        "ROBOMS_THINKING": "high",
        "ROBOMS_WORKSPACE_ROOT": str(tmp_path / "workspaces"),
        "ROBOMS_SQLITE_PATH": str(tmp_path / "roboms.sqlite"),
        "ROBOMS_LOG_DIR": str(tmp_path / "logs"),
        ...
        # cache flip `ROBOMS_NATIVES_CACHE_ENABLED=true` explicitly.
        "ROBOMS_NATIVES_CACHE_ROOT": str(tmp_path / "natives-cache"),
        "ROBOMS_NATIVES_CACHE_ENABLED": "false",
        ...
        "ROBOMS_ISSUE_INDEX_SYNC_SECONDS": "0",
    }
...
def env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    monkeypatch.delenv("ROBOMS_PROVIDER", raising=False)
...
def proxy_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    baseline.pop("ROBOMS_GH_PROXY_URL", None)
    baseline.pop("ROBOMS_GH_PROXY_HMAC_KEY", None)
