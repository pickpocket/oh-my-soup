...
from roboms.host_tools import AbortController, ReleaseToolContext, ToolBindings, build
...
def _release_bindings(
    ...
    *,
    ...
    author_name: str = "roboms-bot",
    author_email: str = "roboms-bot@example.invalid",
    dirty: bool = False,
    ...
    _release_git("config", "user.name", "roboms-bot", cwd=repo_dir)
    _release_git("config", "user.email", "roboms-bot@example.invalid", cwd=repo_dir)
    if create_fix:
    ...
    session_dir = root / ".oms-session-v1.2.3"
    context_dir = root / "context"
    artifacts_dir = root / "artifacts"
