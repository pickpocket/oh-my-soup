...
from roboms.sandbox import LocalGitTransport, SandboxManager, Workspace

pytestmark = pytest.mark.skipif(
    os.environ.get("ROBOMS_PERMISSION_E2E") != "1",
    reason="set ROBOMS_PERMISSION_E2E=1 to run slot-permission e2e tests",
)

_SLOT_ONE = 2001
_SLOT_TWO = 2002
_SHARED_OMP_GID = 2000
_AUTHOR_NAME = "roboms-bot"
_AUTHOR_EMAIL = "roboms-bot@example.invalid"
_REPO = "octo/permission-e2e"


def _require_linux_root_toolchain() -> None:
    if platform.system() != "Linux" or os.geteuid() != 0:
        pytest.skip("slot permission e2e tests require Linux root so subprocesses can drop to oms-N UIDs")
...
def slot_tmp_path() -> Iterator[Path]:
    root = Path(tempfile.mkdtemp(prefix="roboms-permission-e2e-", dir="/tmp"))
    root.chmod(0o755)
    try:
        yield root
    finally:
...
def test_slot_workspace_runs_bun_biome_cargo_and_git_after_root_reentry(
    ...
    stale_bun_cache = first.root / ".oms-xdg" / "cache" / "bun-install" / "root-owned-stale"
...
def _prepare_shared_natives_cache(slot_tmp_path: Path) -> NativesCache:
    """Provision `/data/cache/pi-natives` shape (root:oms, setgid 2770)."""
...
def test_natives_cache_shares_artifacts_across_slot_workspaces(
    ...
    1. A capture from a slot-owned workspace lands in the shared cache with
       group `oms` setgid inheritance so any other slot can read it.
    ...
    # Cache root is setgid `oms`; new files inherit gid `oms` so any slot
    # with `extra_groups=[oms]` can read them.
    ...
    # Slot 2 must be able to read the populated artifacts (group oms + 0660
