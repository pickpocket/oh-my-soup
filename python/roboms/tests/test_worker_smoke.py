...
Runs only when ROBOMS_INTEGRATION=1 and `oms` is available on PATH (or via
ROBOMS_OMP_COMMAND). Spins up:

- a local bare git repo with a trivial failing test,
- a fake GitHub API via httpx.MockTransport that records comments + PRs,
- a real `oms --mode rpc` subprocess driven by `worker.run_task`.
...
INTEGRATION = os.environ.get("ROBOMS_INTEGRATION") == "1"

pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="ROBOMS_INTEGRATION=1 required to run the oms-backed smoke test",
)
...
def test_triage_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from roboms.sandbox import LocalGitTransport, SandboxManager
    from roboms.tasks import triage_issue

    bare = _seed_failing_repo(tmp_path)

    monkeypatch.setenv("ROBOMS_GH_PROXY_URL", "http://gh-proxy.invalid:8081")
    monkeypatch.setenv("ROBOMS_GH_PROXY_HMAC_KEY", "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    monkeypatch.setenv("GITHUB_TOKEN", "")
    monkeypatch.setenv("GITHUB_WEBHOOK_SECRET", "secret")
    monkeypatch.setenv("ROBOMS_BOT_LOGIN", "roboms-bot")
    monkeypatch.setenv("ROBOMS_REPO_ALLOWLIST", "octo/widget")
    monkeypatch.setenv("ROBOMS_WORKSPACE_ROOT", str(tmp_path / "workspaces"))
    monkeypatch.setenv("ROBOMS_SQLITE_PATH", str(tmp_path / "roboms.sqlite"))
    monkeypatch.setenv("ROBOMS_LOG_DIR", str(tmp_path / "logs"))
    monkeypatch.setenv("ROBOMS_TASK_TIMEOUT_SECONDS", "300")
