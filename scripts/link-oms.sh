#!/bin/sh
# Install the dev `oms` wrapper into Bun's global bin directory.
#
...
# `packages/coding-agent/scripts/oms`. See that wrapper's header comment for the
...
# through that error would expand `$(bun pm -g bin)/oms` to `/oms` and try to
# write under `/` — see https://github.com/can1357/oh-my-pi/issues/3701.
set -e

repo_root=$(CDPATH='' cd -- "$(dirname -- "$0")/.." && pwd -P)
target=$repo_root/packages/coding-agent/scripts/oms

if [ ! -x "$target" ]; then
	echo "link-oms: target wrapper not found or not executable: $target" >&2
	exit 1
fi
